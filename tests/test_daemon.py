...
def test_microservice_mode_with_dashboard_addr(serve_mock, mock_fs):
    # Add dashboard_listen_addr to config
    with open("/etc/cos-alerter.yaml", "w") as f:
        config_data = {
            "watch": {
                "down_interval": "4s",
    ...
    # Check that waitress.serve was called twice (API and dashboard servers)
    assert serve_mock.call_count == 2


@unittest.mock.patch("cos_alerter.daemon.waitress.serve")
def test_monolith_mode_without_dashboard_addr(serve_mock, mock_fs):
    main(run_for=0, argv=["cos-alerter"])

    # Check that waitress.serve was called once (single server)
